...
class EffectDslTest {
    ...
    @Test
    fun `schedule timers and outbox`() {
        val at = Instant.parse("2024-01-01T00:00:00Z")
        val effect = Effects.schedule<String, String, String>("reminder", at, payload = "ping")
            .scheduleAfter("follow-up", Duration.ofMinutes(5), payload = "pong")
            .cancelTimer("reminder")
            .outbox(channel = "notifications", message = mapOf("msg" to "hello"))
            .thenNoReply()

        val exec = effect as Effect.Executable<String, String, String>
        assertEquals(3, exec.timers.size)
        assertTrue(exec.timers.any { it is Effect.TimerInstruction.Schedule && it.key == "reminder" })
        assertTrue(exec.timers.any { it is Effect.TimerInstruction.ScheduleAfter && it.key == "follow-up" })
        assertEquals(1, exec.outboxMessages.size)
        assertEquals("notifications", exec.outboxMessages.first().channel)
        assertTrue(exec.timers.any { it is Effect.TimerInstruction.Cancel && it.key == "reminder" })
    }
}
